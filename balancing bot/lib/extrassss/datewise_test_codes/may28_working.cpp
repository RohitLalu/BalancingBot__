...
float Kt = 0;//15; // back calculation for integral term
//float Kp = 81.054, Ki = 604.88, Kd = 2.715;
//float Kp = 449.025, Ki = 3592.2, Kd = 12.21; // tuned for 1.7 factor_speed
float Kp = 66.76, Ki = 534.09, Kd = 1.815; // tuned for 1.7 factor_speed
...
void pid(float pitch,float dt) {
  ...
  // PID control
    if (abs(pitch) > low_threshold && abs(pitch)<mid_threshold) {
      ...
      float I = integral+ Kt*(x);
      float output = Kp * error + I*dt + Kd * derivative;
      output*=(factor);
      lastError = error;
