...
int line_checker_ir(bool ir_left,bool ir_mid,bool ir_right){
  if(ir_left==0 && ir_mid==0 && ir_right==1){return 2;} // turn sharp right
  else if(ir_left==0 && ir_mid==1 && ir_right==0){return 1;} // go straight
  else if(ir_left==0 && ir_mid==1 && ir_right==1){return 3;} // turn right
  else if(ir_left==1 && ir_mid==0 && ir_right==0){return 5;} // turn sharp left
  else if(ir_left==1 && ir_mid==1 && ir_right==0){return 4;} // turn left
  else{return 0;} // stop
}
...
int straight_speed = 25; // Speed for straight movement
int low_turn_speed = 25; // Speed for turns
int  sharp_turn_speed = 50; // Speed for sharp turns
int motorSpeed = 0;
int delayTime = 3600;
...
void motor_speed_ctrl(int line_case,int motorSpeed){
  switch(line_case) {
    case 0: // stability
      ...
      break;
    case 1: // straight
      setMotorSpeed(motorSpeed+(left_or_right*straight_speed),0);
      setMotorSpeed(motorSpeed+(left_or_right*straight_speed),1);
      delayMicroseconds(delayTime);
      break;
    case 2: // turn right
      setMotorSpeed(motorSpeed+(left_or_right*low_turn_speed),0);
      setMotorSpeed(motorSpeed-(left_or_right*low_turn_speed),1);
      delayMicroseconds(delayTime);
      break;
    case 3: // turn sharp right
      setMotorSpeed(motorSpeed+(left_or_right*sharp_turn_speed),0);
      setMotorSpeed(motorSpeed-(left_or_right*sharp_turn_speed),1);
      delayMicroseconds(delayTime);
      break;
    case 4: // turn left
      setMotorSpeed(motorSpeed-(left_or_right*low_turn_speed),0);
    ...
    case 5: // turn sharp left
      ...
      break;
    default: // stability
      Serial.println("Invalid motor control case");
      setMotorSpeed(motorSpeed,0);
      setMotorSpeed(motorSpeed,1);
      delayMicroseconds(delayTime);
      break;
