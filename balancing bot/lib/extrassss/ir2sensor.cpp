...
int line_checker_ir(bool ir_left,bool ir_right){
  if(ir_left==0 && ir_right==1){return 3;} // turn right
  else if(ir_left==1 && ir_right==1){return 1;} // go straight
  else if(ir_left==1 && ir_right==0){return 2;} // turn left
  else{return 0;} // stability
}
...
int low_turn_speed = 50; // Speed for turns
...
switch(debug_reader){
  ...
  case 2: Serial.println("\tTURNING LEFT"); break;
  case 3: Serial.println("\tTURNING RIGHT"); break;
...
void loop() {
  if (imu.Read() && calibrated) {
    ...
    if (tilt_marker ==1){
      ...
      debug_reader = line_checker_ir(digitalRead(pin_left),digitalRead(pin_right));
    }
